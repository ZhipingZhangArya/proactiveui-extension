...
import { SemanticType, SuggestedAction } from "../types/proactive";

interface IntentRequest {
  source: "line" | "selection";
  fileType: "python" | "latex";
  text: string;
...
export class AnthropicIntentClient {
  ...
  async inferIntent(apiKey: string, request: IntentRequest): Promise<IntentResponse | undefined> {
    ...
    const actionIds = (parsed.actionIds ?? []).filter((id): id is keyof typeof ACTION_BY_ID =>
